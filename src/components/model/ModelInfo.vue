<template>
  <div class="model-info-container">
    ...
    <div style="display: flex; justify-content: space-between">
      ...
      <el-button size="mini" type="primary" @click="compareModel()" style="margin:auto 0">算法对比</el-button>

      <el-button size="mini" type="primary" @click="selectModel()" style="margin:auto 0">切换算法</el-button>
    </div>
    ...
    <el-collapse>
      <el-collapse-item title="准确度 Correctness">
        ...
        <div class="metrics"><a>AUC面积</a>：<a class="num">{{ metrics.auc.toFixed(4) }}</a></div>
        <sub-title
        ...
        <div class="metrics"><a>MRR得分</a>：<a class="num">{{ metrics.mean_mrr }}</a>
        </div>
...
'<br/>2）相关性越大的新闻内容排在推荐列表越前的话，则NDCG越大，表明推荐效果越好<br/>（基准数值）协同滤波结果：<br/>nDCG@5:'+baseMetrics['ndcg@5']+'<br/>nDCG@10:'+baseMetrics['ndcg@10']">
      ...
      <el-collapse-item title="覆盖度 Coverage">
        <div class="metrics"><a>被推荐新闻</a>占<a>所有新闻</a>的比例：<a class="num">{{
            metrics.coverage[0] | percentage
          }}</a></div>
        <div class="metrics"><a>被推荐类别</a>占<a>所有类别</a>的比例：<a class="num">{{
            metrics.coverage[1] | percentage
          }}</a></div>
        <div class="metrics"><a>被推荐子类</a>占<a>所有子类</a>的比例：<a class="num">{{
            metrics.coverage[2]| percentage
          }}</a></div>
        <div class="metrics">由系统<a>主动推荐的类别</a>的覆盖率：<a class="num">{{ metrics.coverage[3]| percentage }}</a>
        </div>
    ...
    <!--    <div class="metrics"><a class="metrics-name">ndcg@5</a>: {{metrics['ndcg@5']}}</div>-->
    <!--    <div class="metrics"><a class="metrics-name">ndcg@10</a>: {{metrics['ndcg@10']}}</div>-->

    <CompareDialog :dialog-visible="dialogVisible" @closeDialog="dialogVisible = false" :base-metrics="baseMetrics"
                   :metrics-list="[{name:'nrms',...metrics}]"/>
  </div>
</template>

<script>
import SubTitle from "@/components/SubTitle";
import RocLineChart from "@/components/model/RocLineChart";
import DiversityChart from "./DiversityChart";
import NDCGTable from "./NDCGTable";
import CompareDialog from "./CompareDialog";
import RobustChart from "@/components/model/RobustChart";

export default {
  name: "ModelInfo",
  components: {RobustChart, CompareDialog, NDCGTable, DiversityChart, RocLineChart, SubTitle},
  props: {
    metrics: {
      type: Object,
      default() {
